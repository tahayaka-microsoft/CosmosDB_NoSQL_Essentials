...
## CosmosDBをうまく活用するためのDo/Don't

### DO
- 適切なパーティションキーの設定
- 処理単位での必要RU数、同時接続数、最大データ容量の情報から正しいスループットを見積もる
- アプリ中心のデータモデルの設計によるドキュメントの簡素化（正規化をしない）
- 書込み時に将来必要なことをいろいろ実施する"Write Heavy"に持っていく
- インデックスを必要な範囲に限定
- 物理パーティションの増加を意識した拡張戦略
- Synapse Link・分析ストアの活用
- データプレーンの操作に非同期呼び出しを利用
- 可能であればマルチマスターライトを利用
...
- 分析や集計ワークロードをトランザクションストア(RU/sの設定されたコンテナー)で直接実行する
- データプレーンの操作を同期処理する
- 必要のない整合性"Strong"の使用
<!--
